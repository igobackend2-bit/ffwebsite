...
const SegmentedOTP = ({ value, onChange, length = 6, disabled = false }: {
  ...
  const handlePaste = (e: React.ClipboardData) => {
    const d = (e as unknown as React.ClipboardEvent).clipboardData.getData('text').replace(/\D/g, '').slice(0, length);
    if (d) { onChange(d.padEnd(length, '').slice(0, length)); inputs.current[Math.min(d.length, length - 1)]?.focus(); }
  };
  return (
    <div className="flex gap-2 justify-center" onPaste={handlePaste as unknown as React.ClipboardEventHandler}>
...
export default function AuthModal({ isOpen, onClose }: AuthModalProps) {
  ...
  const handleCompleteSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        await supabase.from('leads').insert({ name: fullName.trim(), phone, source: 'User Signup' });
      } catch { /* non-critical */ }
      toast.success('Welcome to Farmers Factory!');
      onClose();
    } catch (e: unknown) {
      toast.error((e as Error).message || 'Failed to complete signup');
    } finally { setLoading(false); }
  };

  const resetForm = () => { setStep('initial'); setOtp(''); setPhone(''); };
      ...
      <div className="space-y-2">
        ...
        <div className="relative group flex items-center">
          ...
          <input type="tel" inputMode="numeric" maxLength={10}
            ...
            className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 pl-16 pr-6 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all text-sm font-bold placeholder:text-white/20 text-white"
          />
        </div>
      </div>
  ...
  const renderOTP = () => (
    <motion.div key="otp" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
      <div className="text-center space-y-2">
        ...
        <button onClick={resetForm} className="text-primary text-[10px] font-black uppercase tracking-widest hover:underline flex items-center gap-1 mx-auto">
  ...
  const renderDetails = () => (
    <motion.div key="details" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
      <div className="text-center space-y-1 mb-6">
        ...
        <p className="text-white/40 text-sm">One last step to complete your profile.</p>
      </div>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }}
            ...
            <p className="mt-10 text-center text-[9px] font-bold text-white/10 uppercase tracking-[0.4em]">Secure Farm Access · Powered by APITxt OTP</p>
          </motion.div>
        </div>
      )}
    </AnimatePresence>
