              ...
              <div className="relative group">
                ...
                >
                  <div className="w-9 h-9 bg-primary/10 text-primary rounded-full flex items-center justify-center overflow-hidden border border-primary/20">
                    {(user.user_metadata?.full_name?.trim()?.[0] || user.email?.[0] || user.user_metadata?.phone?.[0] || user.phone?.[0] || 'U')?.toUpperCase()}
                  </div>
                  <div className="text-left hidden sm:block">
                    <p className={`text-[9px] font-black uppercase tracking-widest leading-none mb-1 opacity-60`}>{t('nav.profile')}</p>
                    <p className={`text-xs font-bold leading-none truncate max-w-[80px]`}>{user.user_metadata?.full_name?.split(' ')[0] || user.email?.split('@')[0] || user.user_metadata?.phone || user.phone || 'User'}</p>
                  </div>
